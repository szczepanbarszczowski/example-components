...
function dragDiff(value, updating, scale) {
  const tmp = new Value(0);
  const prev = new Value(0);
  const corValue = isIos ? multiply(value, scale) : value;
  return cond(
    updating,
    [set(tmp, sub(corValue, prev)), set(prev, corValue), tmp],
    set(prev, 0),
  );
}
...
// Zoom values of how much image can scale
const MIN_SCALE = 1;
const MAX_SCALE = 5;
...
export {dragDiff, friction, bouncyPinch, bouncy};
